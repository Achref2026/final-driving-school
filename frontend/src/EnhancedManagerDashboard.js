...
const EnhancedManagerDashboard = ({ user, token }) => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [
        ...
        axios.get(`${API}/analytics/school-overview`, { headers })
      ]);

      // Handle dashboard data (includes school info)
      if (dashboardRes.status === 'fulfilled') {
        const dashboardData = dashboardRes.value.data;
        if (dashboardData.user_school) {
          setSchoolData(dashboardData.user_school);
          setSchoolForm({
      ...
      // Handle enrollments and extract students
      if (enrollmentsRes.status === 'fulfilled') {
        const enrollmentData = enrollmentsRes.value.data.enrollments || [];
      ...
      if (analyticsRes.status === 'fulfilled') {
        ...
        // Set basic analytics if API fails
        setAnalytics({
          total_students: students.length,
          total_teachers: teachers.length,
          total_sessions: sessions.length,
          completion_rate: 0,
          average_rating: schoolData?.rating || 0
        });
      }
  ...
  const handleUpdateSchool = async (e) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      // Note: This API endpoint might need to be created in backend
      const response = await axios.put(`${API}/driving-schools/${schoolData.id}`, schoolForm, { headers });
  ...
  const handleRejectStudent = async (enrollmentId) => {
    ...
    try {
      ...
      await axios.post(`${API}/manager/enrollments/${enrollmentId}/refuse`, formData, { headers });
  ...
  const assignStudentToTeacher = async (studentId, teacherId) => {
    try {
      ...
      await axios.post(`${API}/students/${studentId}/assign-teacher`, { teacher_id: teacherId }, { headers });
      ...
      alert(error.response?.data?.detail || 'Failed to assign student to teacher');
    }
  };
  ...
  const pendingApprovals = students.filter(s => s.enrollment_status === 'pending_approval');
  const approvedStudents = students.filter(s => s.enrollment_status === 'approved');
      ...
      <div className="container-fluid px-4">
        ...
        <div className="d-flex justify-content-between align-items-center py-4">
          <div>
            ...
            <p className="text-muted mb-0">
              {schoolData?.name || 'Driving School'} - Complete Management System
            </p>
          </div>
        ...
        <ul className="nav nav-pills mb-4">
          ...
          <li className="nav-item">
            ...
            >
              <i className="fas fa-calendar me-2"></i>Schedules ({sessions.length})
            </button>
          </li>
        </ul>
            ...
            <div className="col-md-3">
              <div className="card bg-warning text-white">
                <div className="card-body">
                  <div className="d-flex justify-content-between">
                    <div>
                      <div className="h4 mb-0">{pendingApprovals.length}</div>
                ...
                <div className="card-body">
                  <div className="row g-3">
                    ...
                    <div className="col-md-3">
                      ...
                      >
                        <i className="fas fa-school fa-2x mb-2"></i>
                        <div>Edit School Info</div>
                      </button>
                    </div>
                  </div>
                </div>
                      ...
                      <tr key={teacher.id}>
                        ...
                        <td>
                          <div className="d-flex gap-1">
                            ...
                            {teacher.can_teach_female && <span className="badge bg-pink" style={{ backgroundColor: '#e91e63' }}>Female</span>}
                          </div>
                        </td>
                        <td>
                          <div className="d-flex align-items-center">
                      ...
                      <tr key={student.id}>
                        ...
                        <td>
                          <select 
                            ...
                            onChange={(e) => assignStudentToTeacher(student.id, e.target.value)}
                          >
                            ...
                            {student.enrollment_status === 'pending_approval' && (
                              <>
                                <button 
                                  ...
                                  onClick={() => handleApproveStudent(student.enrollment_id)}
                                >
                                  <i className="fas fa-check"></i>
                                </button>
                                <button 
                                  className="btn btn-outline-danger"
                                  onClick={() => handleRejectStudent(student.enrollment_id)}
                                >
        ...
        {activeTab === 'school-info' && schoolData && (
          <div className="row g-4">
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      <div className="mb-3">
                        ...
                        <p className="form-control-plaintext">{schoolData.name}</p>
                      </div>
                      <div className="mb-3">
                        <label className="form-label fw-bold">Address</label>
                        <p className="form-control-plaintext">{schoolData.address}</p>
                      </div>
                      <div className="mb-3">
                        <label className="form-label fw-bold">State</label>
                        <p className="form-control-plaintext">{schoolData.state}</p>
                      </div>
                    </div>
                    <div className="col-md-6">
                      <div className="mb-3">
                        <label className="form-label fw-bold">Phone</label>
                        <p className="form-control-plaintext">{schoolData.phone}</p>
                      </div>
                      <div className="mb-3">
                        <label className="form-label fw-bold">Email</label>
                        <p className="form-control-plaintext">{schoolData.email}</p>
                      </div>
                      <div className="mb-3">
                        <label className="form-label fw-bold">Price</label>
                        <p className="form-control-plaintext">{schoolData.price} DA</p>
                      </div>
                    </div>
                    <div className="col-12">
                      <div className="mb-3">
                        <label className="form-label fw-bold">Description</label>
                        <p className="form-control-plaintext">{schoolData.description}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div className="d-flex align-items-center">
                      <span className="text-warning me-1">★</span>
                      {schoolData.rating || 0}/5
                    </div>
                  </div>
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <span>Total Reviews</span>
                    <span className="fw-bold">{schoolData.total_reviews || 0}</span>
                  </div>
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <span>Teachers</span>
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <div className="row g-3">
                    ...
                    <div className="col-6">
                      <div className="metric text-center">
                        <div className="metric-value h3 mb-0 text-info">{sessions.length}</div>
                        <div className="metric-label text-muted">Sessions</div>
                      </div>
                    </div>
                    <div className="col-6">
                      <div className="metric text-center">
                        <div className="metric-value h3 mb-0 text-warning">{schoolData?.rating || 0}/5</div>
                  ...
                  <div className="row">
                    <div className="col-md-3">
                      <div className="stat-card text-center p-3 bg-success bg-opacity-10 rounded">
                        ...
                        <div className="stat-label text-muted">Approved Students</div>
                      </div>
                    </div>
                    <div className="col-md-3">
                      <div className="stat-card text-center p-3 bg-warning bg-opacity-10 rounded">
                        ...
                        <div className="stat-label text-muted">Pending Approval</div>
                      </div>
                    </div>
                    <div className="col-md-3">
                      <div className="stat-card text-center p-3 bg-info bg-opacity-10 rounded">
                        ...
                        <div className="stat-label text-muted">Verified Documents</div>
                      </div>
                    </div>
                    <div className="col-md-3">
